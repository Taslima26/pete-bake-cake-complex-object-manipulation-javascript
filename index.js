...
function cakes(recipe, available) {
  // TODO: insert code
  let availableTotalIngreints = {};
  Object.keys(recipe).forEach((key) => {
    if (available[key] === undefined) {
      available[key] = 0;
    }
    availableTotalIngreints[key] = Math.floor(available[key] / recipe[key]);
  });
  let min = Infinity;
  Object.keys(availableTotalIngreints).forEach((key) => {
    if (availableTotalIngreints[key] < min) {
      min = availableTotalIngreints[key];
    }
  });
  return min;
}
